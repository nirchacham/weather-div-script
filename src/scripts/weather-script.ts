...
import { WeatherDay } from "../interfaces/weather-day"

// Function to create the weather div
async function createWeatherDiv(targetDivId?: string) {
  ...
  searchBtn.addEventListener("click", async () => {
    ...
    if (cityValue) {
      try {
        const apiData = await getWeatherInsightsFromApi(cityValue)
        const avgDatasets = await calculateDailyAverages(apiData)
        displayDailyAverages(avgDatasets, temperatureContainer)
      } catch (error) {
        console.error("Error: ", error.message)
        alert(
          "An error occurred while fetching weather data, possibly entered unvalid city."
        )
      }
    } else {
...
async function calculateDailyAverages(data: WeatherData[]) {
  const dailyAverages: {
    [day: string]: WeatherDay
  } = {}
    ...
    if (!dailyAverages[day]) {
      ...
      dailyAverages[day] = {
        temperature: Math.round(
          (currentDay.temperature + temperature) / (currentDay.count + 1)
        ),
...
function displayDailyAverages(
  averages: {
    [day: string]: { temperature: number; description: string; icon: string }
  },
...
async function getWeatherInsightsFromApi(locationInput: string) {
  ...
  try {
    const result = await fetch(
      `${apiUrl}${apiUrlSuffix}&days=14&key=${weatherBitApi}`
    )
    if (!result.ok) {
